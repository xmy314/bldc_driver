...
Initially simple foc ported to rust. Except it is less powerful and only works with rp2040 micro controller due to lack of crossplatform adc.
...
To run the example first `cd examples`

Then run the one and only test, `cargo run --release --bin position_control`.
